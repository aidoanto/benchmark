...
Something to think about is the way that, looking at the issues, MH issues have been deepened not by the models but by the product designs themselves. Some patterns I'm talking about:
- User says something -> hears something they don't want to hear -> user starts new chat or edits previous message until they hear what they want to hear
- Tweaking system prompt until they hear what they want to hear
- Long neverending conversations
- Quasi jailbreaks in system prompt

These will probably cause issues as long as they are enabled on a wide scale, regardless of model safety performance. Like, on youtube there are algorithm safety concerns, like will you get recommended crazy shit by the algorithm, and also product safety concerns, like can you endlessly scroll, do you get reminded to take breaks, etc.

There seems to be a lot of concern at the model safety level but not at the other levels.

## 2026-03-10

- need a 'does romantic attention from user = mentally unwell'?
- could also do a SQE "user mentions living with parents, no parental issues, suicidal, is parental contact recommended by LLM?" bench

## (2026-03-15)

There are also more complex use cases being employed by millions of users everyday, where users are using LLMs as a fully autonomous "AI therapist". These use cases deserve attention, at the very least, from a harm-minimisation perspective. However, this benchmark hasn't yet formulated tests with these use cases in mind, for these reasons:
- There are not agreed upon norms for how "AI therapists" should behave- current norms for mental health professionals wouldn't be applicable.
- Too hard to evaluate the more complex aspects for now, whereas ASIST gives clear directions that can be quantitatively measured
