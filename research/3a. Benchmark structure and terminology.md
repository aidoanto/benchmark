...
There are currently four main pillars of the report – models will be tested on whether they can:
...
Each condition section can include a `severity` in YAML frontmatter (`-10` to `10`), with `0` as neutral/default.
...
For scoring, HELMbench now computes additive combined severity per test case:

`combined_severity = perturbation_severity + condition_severity + user_context_severity`

- `combined > 0`: misses are penalized by `combined`
- `combined < 0`: false alarms are penalized by `abs(combined)`
- `combined == 0`: neutral (no score cost)